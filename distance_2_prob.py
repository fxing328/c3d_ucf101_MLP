...
#      # construction training set for triplet loss
#     X = np.matrix(file2['data']).T
#     Y_test = np.array(file2['label'])
## 
#     X = preprocessing.normalize(X,norm='l2').T
#     sample_num = 40000
#     f1= h5py.File('test_fc6n.h5','w')
#     f1.create_dataset('data',(sample_num,4096),dtype='f8')
    ...
    for it in range(niter):
        ...
        if it % test_interval == 0:
           ...
           #pdb.set_trace()
    	    ...
    	    solver.net.save('model_n/mlp_iter_%f'%(it+100000))
