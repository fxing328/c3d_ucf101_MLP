...
if __name__ == '__main__':
    file1 = pkl.load(open('feature_c3d_train_fc8','rb'))
# # construction training set for triplet loss
  ...
  #X = preprocessing.normalize(X,norm='l2').T
    ...
    file2 = pkl.load(open('feature_c3d_test_fc8','rb'))
  #    # construction training set for triplet loss
 ...
 #X_test = preprocessing.normalize(X_test,norm='l2').T
    ...
    sample_num = 100000 
    f= h5py.File('train_fc8.h5','w')
    f.create_dataset('data',(sample_num,202),dtype='f8')
    ...
    for i in range(sample_num):
       ...
       f['data'][i] = np.concatenate((X_1,X_2),axis=0).T
       f['label'][i] = train_label 
    f.close()

 
    sample_num = 40000
    f1= h5py.File('test_fc8.h5','w')
    f1.create_dataset('data',(sample_num,202),dtype='f8')
    ...
    for i in range(sample_num):
       ...
       f1['data'][i] = np.concatenate((X_1,X_2),axis=0).T
...
#
    ...
    for it in range(niter):
	    ...
	    #pdb.set_trace()
    	    ...
    	    solver.net.save('model/mlp_iter_%f'%(it+20000))
